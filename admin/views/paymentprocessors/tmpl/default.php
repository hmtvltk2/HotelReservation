...
if ($saveOrder)
{
    $saveOrderingUrl = 'index.php?option=com_jhotelreservation&task=paymentprocessors.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_jhotelreservation&view=paymentprocessors');?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="editcell">
		<fieldset>
			...
			<?php
			$paymentprocessors = count( $this->items );
			if( $this->state->get('filter.hotel_id') > 0 && $paymentprocessors > 0 )
			{
				?>
				...
				<table class="table adminlist" id="itemList">
					...
					<tr>
						...
						<Th width='15%' class="nowrap">
							<B>
								<?php echo JHtml::_( 'grid.sort', 'LNG_NAME', 'r.name', $listDirn, $listOrder ); ?>
							</B>
						</Th>
					...
					for ( $i = 0; $i < $paymentprocessors; $i ++ )
					{
						...
						<TR class="row<?php echo $i % 2 ?>"
							...
							<td class="order nowrap   hidden-phone">
								<?php
								//$canChange = $user->authorise('core.edit.state', '.paymentprocessors.' . $room->room_id);
								$iconClass = '';
								if ( ! $canChange )
								{
									$iconClass = ' inactive';
							...
							{ ?>
								<td class=" ">
									<a class="btn btn-micro hasTooltip" href="javascript:void(0);"
									   onclick="return listItemTask('cb<?php echo $i ?>','paymentprocessors.changeState')"
							...
							{ ?>
								<td class="  hidden-phone">
									<a class="btn btn-micro hasTooltip" href="javascript:void(0);"
									   onclick="return listItemTask('cb<?php echo $i ?>','paymentprocessors.changeFrontState')"
									   ><i class="icon-unpublish"></i></a>
								</td>
							<?php }
							else
	...
	<script language="javascript" type="text/javascript">
		...
		Joomla.submitbutton = function(task) {
			if (task != 'paymentprocessors.delete' || confirm('<?php echo JText::_('LNG_ARE_YOU_SURE_YOU_WANT_TO_DELETE', true,true);?>')) {
